...
def generate_prediction_beta(ticker, prediction_lag, print_eval):
    ...
    # BETA-drop
    beta_drop = pd.DataFrame(hist_spy[hist_spy["pct_change_close"] <= BETA]["pct_change_close"])
    beta_duration = pd.Series(beta_drop.index) - pd.Series(beta_drop.index).shift(1) 
    # date attached is the date of event happening, duration is time since last event
    beta_drop = beta_drop.assign(abs_duration=beta_duration.values)
    beta_drop = beta_drop.iloc[1:]

    # insert column of dates of last event trigger
    beta_drop["prev_date"] = beta_drop.index - beta_drop["abs_duration"]
    ...
    for idx, row in beta_drop.iterrows():
        ...
        active_days = hist_spy.loc[start_date : end_date].shape[0]
        avg_ROC = hist_spy.loc[start_date : end_date]["pct_change_close"].ewm(span=active_days).mean()[-1]

        avg_VIX = VIX_cropped.loc[start_date : end_date].ewm(span=active_days).mean()
    ...
    drop_df = pd.DataFrame({'duration': active_days_series, 'acc_ROC': acc_ROC_series, 'avg_ROC': avg_ROC_series, 'exp_avg_VIX': exp_avg_VIX_series, 'rsi': rsi_series, 'psy': psy_series})

    # fit with weibull aft
    aft = CoxPHFitter()
    aft.fit(drop_df, duration_col='duration')
    ...
    print(f"days since last {BETA}%-drop:", active_days_test)
    ...
    prediction['delta_probability'] = prediction[0].diff(-1) * 100
...
if __name__ == "__main__":
    print(f"probability of {BETA}%-drop in the next day:", generate_prediction_beta("SPY", 0, False))